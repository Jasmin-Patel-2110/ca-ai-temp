...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success({
          message: result.message || "Your account has been created successfully. Please sign in.",
          theme: "colored"
        });
        router.push("/login");
      } else {
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-primary/10 items-center justify-center p-12 relative overflow-hidden">
        ...
        >
          ...
          <p className="text-muted-foreground text-lg leading-relaxed">
            Streamline your accounting workflow with AI-powered invoice processing.
          </p>
        </motion.div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-card">
        ...
        >
          <div className="text-center lg:text-left">
            ...
            <p className="text-muted-foreground mt-2">Start processing invoices in minutes</p>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email">Name</Label>
              <Input id="name" placeholder="Joan Doe" className="h-11" {...register("name")} />
              {errors.name && <p className="text-sm text-destructive font-medium">{errors.name.message}</p>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" placeholder="rajesh@example.com" className="h-11" {...register("email")} />
              {errors.email && <p className="text-sm text-destructive font-medium">{errors.email.message}</p>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="companyName">Company Name</Label>
              <Input id="companyName" placeholder="Rajesh & Co." className="h-11" {...register("companyName")} />
            ...
            <Button type="submit" className="w-full h-11 text-base font-semibold shadow-lg mt-2" disabled={isLoading}>
              {isLoading ? (
